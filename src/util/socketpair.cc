...
{
  int pipe[ 2 ];
  CheckSystemCall( "socketpair", socketpair( AF_UNIX, SOCK_DGRAM, 0, pipe ) );
...
void UnixDomainSocket::send_fd( FileDescriptor & fd )
{
  ...
  *reinterpret_cast<int *>( CMSG_DATA( control_message ) ) = fd.fd_num();
  ...
  if ( 0 != CheckSystemCall( "sendmsg", sendmsg( fd_num(), &message_header, 0 ) ) ) {
...
FileDescriptor UnixDomainSocket::recv_fd( void )
{
  ...
  if ( 0 != CheckSystemCall( "recvmsg", recvmsg( fd_num(), &message_header, 0 ) ) ) {
