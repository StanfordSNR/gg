...
def generate_batch(start, end, quality):
        ...
        for i in range(start + 1, end + 1):
            ...
            if i == (start + 1):
                print("{i}.ivf: {i}.y4m {i}-0.ivf {j}-0.state".format(i=name, j=prev_name))
                print(make_command(XC_ENC_FIRST_FRAME.format(input=name, output=name,
                                                                  source_state="%s-0" % prev_name,
                                                                  input_pred="%s-0" % name)))
            else:
                print("{i}-1.ivf: {i}.y4m {i}-0.ivf {j}-0.state".format(i=name, j=prev_name))
                print(make_command(XC_ENC_FIRST_FRAME.format(input=name, output="%s-1" % name,
                                                                  source_state="%s-0" % prev_name,
                                                                  input_pred="%s-0" % name)))
            ...
            if i == (start + 1):
                print("{i}-1.state: {i}.ivf {j}-0.state".format(i=name, j=prev_name))
                print(make_command(XC_DUMP_1.format(input=name, input_state="%s-0" % prev_name,
                                                         output="%s-1" % name)))
        ...
        for i in range(start + 2, end + 1):
            ...
            print(make_command(XC_ENC_REBASE.format(output=name, input=name, source_state="%s-1" % prev_name,
                                                         input_pred="%s-1" % name, pred_state="%s-0" % prev_name)))

            if i != end:
                print("{i}-1.state: {i}.ivf {j}-1.state".format(i=name, j=prev_name))
                print(make_command(XC_DUMP_1.format(input=name, input_state="%s-1" % prev_name, output="%s-1" % name)))
                print()
